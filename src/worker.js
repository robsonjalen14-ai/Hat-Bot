...
const SITE_BACKFILL_ORIGINS = [
  "https://hat-manifest.vyro.workers.dev",
  "https://hat-manifest.co.in",
  "https://www.hat-manifest.co.in"
];
...
function gameEmbed(game, source = null) {
  ...
  const result = {
    ...
    color: SUCCESS,
    fields,
    footer: { text: "Hat Manifest Manifest Tool" }
  };
...
function genWebsiteUrl(appId) {
  return `https://hat-manifest.vyro.workers.dev/hat-manifest-gen?appid=${encodeURIComponent(appId)}`;
}
...
function packageButtons(downloadUrl, appId) {
  ...
  components.push({
    type: 2,
    style: 5,
    label: "Open on Hat Manifest Gen",
...
async function botStatusEmbed(env) {
  ...
  return embed("Hat Manifest Bot Status", [
...
async function sendResult(env, interaction, result) {
  if (typeof result === "string") {
    await sendInteractionFollowup(env, interaction, "", {
      embeds: [messageEmbed("Hat Manifest", result)]
    });
    return;
  }
  const embeds = [...(result.embeds || [])];
  if (result.content) embeds.unshift(messageEmbed("Hat Manifest", result.content));
...
async function notifyUserAction(env, interaction, target, action, reason = "Not provided", fields = []) {
  ...
  await dmUser(env, userId, `Hat Manifest Notice: ${action}`, [
...
async function handleGenCommand(env, interaction, ctx = null) {
  ...
  await editOriginalInteraction(env, interaction, "", null, {
    embeds: [messageEmbed("🔍 Searching Hat Manifest Repository...", "Please wait while the requested manifest package is located and verified.", MOD)],
  });
  ...
  const genlog = async () => {
    try {
      ...
      if (result.source === "Used Hat Manifest Repo") {
  ...
  if (packageBytes > MAX_DIRECT_INTERACTION_UPLOAD_BYTES) {
    await editOriginalInteraction(env, interaction, "", null, {
      embeds: [messageEmbed(
        "Package Too Large",
        `Hat Manifest found the package for AppID **${appId}**, but the generated ZIP is ${(packageBytes / 1024 / 1024).toFixed(1)} MB after bundling manifests. Open it in Hat Manifest Gen to generate and download it from the website.`,
        MOD
      )],
...
async function handleStaffDispatch(env, interaction) {
  await requireModerator(env, interaction);
  const action = subcommandName(interaction);

  if (action === "role") return handleStaffRoleSet(env, interaction);

  if (action === "add") return handleAdminAdd(env, interaction);
  if (action === "remove") return handleAdminRemove(env, interaction);
  if (action === "list") return handleAdminList(env, interaction);
...
async function handleAdminCommand(env, interaction) {
  ...
  if (sub === 'manifest') {
    var appId = getOption(commandOptions(interaction), 'appid');
    if (!appId) throw new Error('AppID is required.');
    var gameInfo = await searchAppId(appId);
    var manifestInfo = await getManifest(env, appId);
    return { embeds: [messageEmbed("📦 Manifest for " + appId, gameInfo + '\n' + manifestInfo)] };
  }
  if (sub === 'genadd') {
    var targetUser = getOption(commandOptions(interaction), 'user');
    var count = parseInt(getOption(commandOptions(interaction), 'count'), 10) || 1;
    var targetId = targetUser ? String(targetUser).replace(/[<@!>]/g, '').trim() : null;
    if (!targetId) throw new Error('User is required.');
    var newRemaining = await addGenLimit(env, targetId, count);
    await storeAndSendModLog(env, { action: 'Gen Limit Added', moderator: getUser(interaction), target: targetId, reason: '+' + count + ' generations' });
...
async function handleChannelCommand(env, interaction) {
  ...
  if (action === "list") {
    ...
    return {
      embeds: [embed("Hat Manifest Channels", channels.map((item) => ({
        name: item.label,
...
async function handleClearWarns(env, interaction) {
  ...
  await putStored(env, "warnings", warnings);
  await notifyUserAction(env, interaction, target, "Warnings Cleared", "Your Hat Manifest warnings were cleared.");
...
async function handleNuke(env, interaction) {
  ...
  await sendChannelMessage(env, created.id, "Channel nuked and recreated by Hat Manifest.");
...
async function handleMail(env, interaction) {
  ...
  if (action === "send" || action === "channel") {
    const subject = String(commandOption(interaction, "subject", "Hat Manifest Mail")).trim();
    const message = String(commandOption(interaction, "message", "")).trim();
...
async function runCommand(env, interaction) {
  switch (interaction.data.name) {
    ...
    case "refresh": return handleRefresh(env, interaction);
      case "staff": return handleStaffDispatch(env, interaction);
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (request.method === "GET") {
      return text("Hat Manifest Discord bot is running.");
    }
    if (request.method !== "POST") {
