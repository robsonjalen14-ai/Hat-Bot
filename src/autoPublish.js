...
const DEFAULT_FALLBACK_MANIFEST_REPOSITORIES = "";
...
export function createAutoPublishedGameEmbed({ appId, fileName, game }) {
  ...
  const embed = {
    ...
    description: hasMetadata
      ? "A new manifest has been published to Hat Manifest."
      : undefined,
    fields: hasMetadata ? [
      ...
      field("⬆ Uploaded By", "Hat Manifest Bot", true),
      field("☁ Published", "✅ Database 1\n✅ Database 2", false)
    ] : [
      field("App ID", `\`${appId}\``, true),
      field("File", `\`${fileName}\``, true),
      field("Uploader", "Hat Manifest Bot", true)
    ],
...
export async function autoPublishExternalPackage(env, appId, result, game = null, genlogData = null) {
  ...
  try {
    const published = await publishNewManifest(env, appId, fileName, result.bytes, "Hat Manifest Bot");
...
export async function autoPublishExternalManifest(env, fileName) {
  ...
  if (!repositories.length) {
    return { published: false, fileName: cleanFileName, reason: "no-sources-configured" };
  }
