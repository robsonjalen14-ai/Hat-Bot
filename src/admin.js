...
function staffRoleKey(g) { return "staff-role:" + g; }
...
export async function handleStaffRoleSet(env, interaction) {
  ...
  await putStored(env, staffRoleKey(interaction.guild_id), roleId);
  return { embeds: [createEmbed({ title: "Staff Role Set", description: "Staff role has been set to <@&" + roleId + ">.", color: BRAND.colors.success })] };
}
