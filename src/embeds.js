...
export const BRAND = {
  name: "Hat Manifest Manifest Tool",
  cyan: 0x05fff7,
  purple: 0x8b5cf6,
  neutral: 0x2f3441
};

const WEBSITE_URL = "https://hat-manifest.vyro.workers.dev/";
const WEBSITE_LOGO_URL = "https://hat-manifest.vyro.workers.dev/images/icon-512.png";
...
function sourceLabel(source) {
  ...
  if (/hat-manifest|repo/i.test(source || "")) return "Hat Manifest Repo";
...
export function createWebsiteEmbed() {
  return {
    title: "🌐 HAT MANIFEST WEBSITE",
    url: WEBSITE_URL,
    description: [
      "Everything Hat Manifest in one place.",
      "",
      "━━━━━━━━━━━━━━",
      "📦 Download Hat Manifest",
      "⚙️ Use Hat Manifest Gen",
      "📖 Read the Guide",
      "💬 Join the Community",
      "━━━━━━━━━━━━━━",
      "",
    ...
    timestamp: new Date().toISOString(),
    footer: { text: "Powered by Hat Manifest" }
  };
}


export function createGenLogEmbed({ game, source, manifestSource, manifestCount, elapsedMs, user, backfillStatus }) {
  const appId = safeText(game?.appId);
  const title = safeText(game?.name, `Steam App ${appId}`);
  const publisher = publisherText(game);
  const release = releaseText(game);
