...
function baseJobEmbed(job, game, options = {}) {
  ...
  const embed = {
    ...
    description: options.description ?? (isFix
      ? "A repair request has been submitted.\n\nThis operation may update or replace manifests inside Hat Manifest repositories."
      : "A new request has entered the Hat Manifest queue."),
    fields: [
...
function alreadyAvailableEmbed(appId, game) {
  return gameThumb({
    ...
    description: "Hat Manifest checked both repositories and found existing manifest files for this App ID.",
    fields: [
...
function announcementEmbed(job, game) {
  ...
  const embed = {
    ...
    description: hasMetadata
      ? (isFix ? "A manifest repair has been published to Hat Manifest." : "A new manifest has been published to Hat Manifest.")
      : undefined,
    fields: hasMetadata ? [
    ...
    footer: { text: isFix ? "Powered by Hat Manifest Repair System" : "Powered by Hat Manifest" },
    timestamp: new Date().toISOString()
  };
...
export async function handleManifestRequestCommand(env, interaction) {
  ...
  await editOriginalInteraction(env, interaction, "", null, {
    embeds: [{
      ...
      description: "Your request has been submitted.\n\nOnce someone uploads a valid manifest,\nHat Manifest will publish automatically.",
      footer: { text: "Hat Manifest Request System" },
      timestamp: new Date().toISOString()
    }]
  });
}
...
export async function handleManifestStatusCommand(env, interaction) {
  ...
  return {
    embeds: [gameThumb({
      ...
      footer: { text: "Hat Manifest Status" },
      timestamp: new Date().toISOString()
    }, game)]
  };
}
...
export async function handleManifestHistoryCommand(env, interaction) {
  ...
  return {
    embeds: [gameThumb({
      ...
      footer: { text: "Hat Manifest History" },
      timestamp: new Date().toISOString()
    }, game)]
  };
}
...
export async function handleStatsCommand(env) {
  ...
  return {
    embeds: [{
      ...
      footer: { text: "Hat Manifest Statistics" },
      timestamp: new Date().toISOString()
    }]
  };
}
...
export function createMailEmbed({ subject, message, sender }) {
  return {
    ...
    footer: { text: "Powered by Hat Manifest Mail System" },
    timestamp: new Date().toISOString()
  };
}
...
export function mailComponents(options = {}) {
  const row = [];
  if (options.website) {
    row.push({ type: COMPONENT.BUTTON, style: 5, label: "Website", emoji: { name: "🌐" }, url: "https://hat-manifest.vyro.workers.dev/" });
  }
