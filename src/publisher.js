...
function manifestVaultConfig(env) {
  const owner = env.MANIFEST_VAULT_OWNER || env.GITHUB_OWNER;
  const repo = env.MANIFEST_VAULT_REPO;
  const branch = env.MANIFEST_VAULT_BRANCH || env.GITHUB_BRANCH || "main";
  if (!owner || !repo) throw new Error("MANIFEST_VAULT_OWNER and MANIFEST_VAULT_REPO must be configured.");
  if (!env.GITHUB_TOKEN) throw new Error("GITHUB_TOKEN is not configured.");
  return { owner, repo, branch, token: env.GITHUB_TOKEN };
}
...
export async function healthCheck(env) {
  const checks = {
    ...
    hatManifestDatabase: false,
    manifestVault: !(env.MANIFEST_VAULT_OWNER || env.MANIFEST_VAULT_REPO)
  };
  ...
  try {
    ...
    checks.hatManifestDatabase = true;
  } catch (error) {
    errors.push(`Hat Manifest database: ${error.message}`);
  }

  if (env.MANIFEST_VAULT_OWNER || env.MANIFEST_VAULT_REPO) {
    try {
      const config = manifestVaultConfig(env);
      await githubRequestWithConfig(config, `/repos/${config.owner}/${config.repo}`);
      checks.manifestVault = true;
    } catch (error) {
      errors.push(`Manifest Vault: ${error.message}`);
    }
  }
