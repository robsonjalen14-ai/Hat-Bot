...
test("createManifestEmbed returns one premium result card with image and badges", () => {
  ...
  assert.match(embed.description, /Hat Manifest Repo/);
...
test("createWebsiteEmbed and website button point at the public website", () => {
  ...
  assert.equal(embed.url, "https://hat-manifest.vyro.workers.dev/");
  assert.equal(embed.title, "🌐 HAT MANIFEST WEBSITE");
  assert.match(embed.description, /Everything Hat Manifest in one place/i);
  assert.equal(embed.thumbnail.url, "https://hat-manifest.vyro.workers.dev/images/icon-512.png");
  assert.equal(components[0].components[0].label, "🌐 Visit Website");
  assert.equal(components[0].components[0].url, "https://hat-manifest.vyro.workers.dev/");
});
