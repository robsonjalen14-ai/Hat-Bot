...
test("autoPublishExternalPackage uploads external ZIP to both databases, backfills bundled manifests, and announces Hat Manifest Bot", async () => {
  ...
  globalThis.fetch = async (url, options = {}) => {
    ...
    if (method === "GET" && value === "https://api.github.com/repos/robsonjalen14-ai/hat-database/contents/2215201_123456789.manifest?ref=main") {
      return new Response("not found", { status: 404 });
    }
    if (method === "PUT" && value === "https://api.github.com/repos/robsonjalen14-ai/hat-database/contents/2215201_123456789.manifest") {
  ...
  try {
    const result = await autoPublishExternalPackage({
      ...
      MANIFEST_VAULT_OWNER: "robsonjalen14-ai",
      MANIFEST_VAULT_REPO: "hat-database",
    ...
    assert.match(JSON.stringify(announcements[0]), /Hat Manifest Bot/);
  } finally {
...
test("autoPublishExternalManifest copies fallback manifest into ManifestVault", async () => {
  ...
  globalThis.fetch = async (url, options = {}) => {
    ...
    if (method === "GET" && value === "https://raw.githubusercontent.com/example/fallback/main/228980_111.manifest") {
      return new Response(new TextEncoder().encode("manifest"), { status: 200 });
    }
    if (method === "GET" && value === "https://api.github.com/repos/robsonjalen14-ai/hat-database/contents/228980_111.manifest?ref=main") {
      return new Response("not found", { status: 404 });
    }
    if (method === "PUT" && value === "https://api.github.com/repos/robsonjalen14-ai/hat-database/contents/228980_111.manifest") {
  ...
  try {
    const result = await autoPublishExternalManifest({
      GITHUB_TOKEN: "token",
      GITHUB_OWNER: "robsonjalen14-ai",
      GITHUB_REPO: "hat-database",
      MANIFEST_VAULT_OWNER: "robsonjalen14-ai",
      MANIFEST_VAULT_REPO: "hat-database",
      MANIFEST_FALLBACK_URLS: "https://raw.githubusercontent.com/example/fallback/main"
    }, "228980_111.manifest");
