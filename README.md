...
- `/gen appid:<number>` searches the Hat Manifest database in order:
  1. Database 1 (`hat-database/database-1`)
  2. Database 2, if configured
...
- `/website` opens the official Hat Manifest website.
...
- `/request` checks Hat Manifest repositories before creating a request, then supports direct-URL upload and 60-second chat attachment upload publishing to both database folders.
...
- `TICKET_CATEGORY_ID`
- database URLs
- GitHub owner/repo/branch for Hat Manifest Database
